...
std::vector<uint8_t> UpgradeProcess::buildTransferPacket(const std::vector<uint8_t>& firmwareData, uint8_t seqNo) {
...
bool UpgradeProcess::transferData(i2cDriverInfo &info,uint32_t addrInBinFile,uint32_t screenFirmwareDecLen,const std::string &module,uint16_t &outChecksum)
{
    ...
    while(currentFrameIndex < transferCount) {
        ...
        transferDataReqParam = buildTransferPacket(firmwareData, seqNo);
        for (int i = 0; i < transferDataReqParam.size(); i++) { // 只显示前16个字节，避免输出过多
            std::cout << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(firmwareData[i]) ;
        }
        std::cout<<std::dec<<std::endl;
      
        // 输出I2C请求命令
        std::cout << "transferData [I2C请求] 命令: 0x60 0x86 0x00 0x83 0x" << std::hex << static_cast<int>(seqNo) << std::dec << std::endl;
                //   << ", 数据长度: " << transferDataReqParam.size() - 6 
                //   << ", CRC: 0x" << std::hex << std::setw(4) << std::setfill('0') << crc 
                ...
                while (readRetryCount < MAX_READ_RETRIES) {
                    if(i2cProcessHandler::i2cReadHandler(info, 0x61, sizeof(uint8_t), transferDataResp, sizeof(transferDataResp))) {
                        ...
                        if(transferDataResp[3] == 0x00) { //OK
                            ...
                            currentFrameIndex++;
                            seqNo = (seqNo % 255) + 1;
                            std::cout<<"seqNo="<<static_cast<int>(seqNo)<<std::endl;
                            // 跳出所有重试循环，继续处理下一帧
