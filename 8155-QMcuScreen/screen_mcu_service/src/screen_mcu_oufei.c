...
static int set_request_oufei(void *screen_mcu_drv, char *request){
...
static int get_reply_oufei(void *screen_mcu_drv, char *reply_msg){
	struct screen_mcu_drv *screen_mcu_p = (struct screen_mcu_drv *)screen_mcu_drv;
	unsigned nbytes=0;
	pthread_mutex_lock(&screen_mcu_p->lock);
...
static uint8_t mcu_calc_crc8(uint8_t *data, uint8_t len)
{
    uint8_t sum = 0;
    for(int i=0; i<len; i++){
        sum = sum + data[i];
...
static int do_control_action(struct screen_mcu_drv *screen_mcu_p, uint8_t power, uint8_t tilt, uint8_t backlight_power, uint8_t backlight){
    /*/
	...
	return i2c_write_bat(screen_mcu_p, screen_mcu_p->i2caddr, write_byte, 5);
	/*/
	...
	write_byte[4] = 0x00;
	write_byte[5] = 0x00;
	write_byte[6] = mcu_calc_crc8(write_byte, 6);
...
static int getkeystatus(struct screen_mcu_drv *screen_mcu_p, uint8_t* tilt_degree, uint8_t* powerkey_status){
    int ret;
    uint8_t buf[3] = {0};
    ret = i2c_read_bytes(screen_mcu_p, screen_mcu_p->i2caddr, 0x44, buf, 4);
    if(!ret){
        *tilt_degree = buf[0];
...
static int start_send_data(struct screen_mcu_drv *screen_mcu_p,uint8_t type, int data_len, int blocks_num){
     ...
     write_byte[0] = 0xc7;
     write_byte[1] = type;
     write_byte[2] = (blocks_num>>8)&0xff;
...
static int handle_request(struct screen_mcu_drv *screen_mcu_p){
    ...
    struct oufei_info *oufei_info_p = screen_mcu_p->priv_data;
	if(!strcmp(screen_mcu_p->request, "lcd power on")){
		do_control_action(screen_mcu_p, 2, 0, 0, 0);				
	}else if(!strcmp(screen_mcu_p->request, "lcd power off")){
		do_control_action(screen_mcu_p, 1, 0, 0, 0);
	}else if(!strcmp(screen_mcu_p->request, "lcd tilt forward")){
		do_control_action(screen_mcu_p, 0, 1, 0, 0);
	}else if(!strcmp(screen_mcu_p->request, "lcd tilt backward")){
		do_control_action(screen_mcu_p, 0, 2, 0, 0);
	    ...
	    if(!getkeystatus(screen_mcu_p, &tilt_degree, &powerkey_status)){
		 ...
		 snprintf(screen_mcu_p->reply, 32, "%d.%d", oufei_info_p->sw_ver[0], oufei_info_p->sw_ver[1]);
	}else if(!strcmp(screen_mcu_p->request, "lcd get supplier hver")){
		 snprintf(screen_mcu_p->reply, 32, "%d.%d", oufei_info_p->hw_ver[0], oufei_info_p->hw_ver[1]);
	}else if(!strcmp(screen_mcu_p->request, "lcd get supplier blver")){
		 snprintf(screen_mcu_p->reply, 32, "%d.%d", oufei_info_p->bl_ver[0], oufei_info_p->bl_ver[1]);
...
static int get_version_info(struct screen_mcu_drv *screen_mcu_p){
    ...
    if(!ret){
        //oufei_info_p->sw_ver[0] = buf[0];
        //oufei_info_p->sw_ver[1] = buf[1];
        //oufei_info_p->hw_ver[0] = buf[2];
        //oufei_info_p->hw_ver[1] = buf[3];
        memcpy(oufei_info_p->hw_ver, &buf[0], 2);
        memcpy(oufei_info_p->sw_ver, &buf[2], 2);
        memcpy(oufei_info_p->bl_ver, &buf[4], 2);
        oufei_info_p = screen_mcu_p->priv_data;
        info_out("%s, busnum:%d, i2caddr:0x%02x, %d.%d, %d.%d\n", __func__, screen_mcu_p->busnum, screen_mcu_p->i2caddr, oufei_info_p->sw_ver[0], oufei_info_p->sw_ver[1], oufei_info_p->hw_ver[0], oufei_info_p->hw_ver[1]);
     }
    return ret;
    /*/
    return 0;
...
static int heart_beat(struct screen_mcu_drv *screen_mcu_p){
    int ret = -1;
    do{
        uint8_t write_byte[1] = {0};
	    write_byte[0] = 0x00;
	    ret = i2c_write_bat(screen_mcu_p, screen_mcu_p->i2caddr, write_byte, 1);
	    if(ret){
	        break;
	    }
	    //usleep(10000);
	    uint8_t buf[1] = {0};
	    ret = i2c_read_bytes_direct(screen_mcu_p, screen_mcu_p->i2caddr, buf, 1);
	    if(ret){
	        break;
	    }
	    //info_out("%s, busnum:%d, i2caddr:0x%02x,buf[0]:%02x\n", __func__, screen_mcu_p->busnum, screen_mcu_p->i2caddr, buf[0]);
	    return ret;
	}while(0);
	err_out("%s, busnum:%d, i2caddr:0x%02x, heart_beat erro \n", __func__, screen_mcu_p->busnum, screen_mcu_p->i2caddr);
    return ret;
}
	...
	do{
	    if(screen_mcu_p->status){
		    ...
		    if(screen_mcu_p->pwr_chg){
		        if(!do_control_action(screen_mcu_p, 0, 0, screen_mcu_p->bl_on?2:1, 0)){
		            screen_mcu_p->pwr_chg = 0;
		        }
		    }
		    if(screen_mcu_p->bkl_chg){
		        if(!do_control_action(screen_mcu_p, 0, 0, 0, screen_mcu_p->bl)){
		    ...
		    heart_beat(screen_mcu_p);
		}
		wait_time(screen_mcu_p, 500000);
...
static int mcu_ts_read_data(struct screen_mcu_drv *screen_mcu_p){
    int ret = -1;
    struct oufei_info *oufei_info_p =  screen_mcu_p->priv_data;
    screen_mcu_tp_t *tp_info = oufei_info_p->tp_info;
    uint8_t buf[64];
    int notify_sem_value;
    ret = i2c_read_bytes_direct(screen_mcu_p, screen_mcu_p->i2caddr, buf, 16);
    if(!ret && buf[0] == 0x41){
		...
		next_tp_len = buf[14];
		if(pc > 2){
			i2c_read_bytes_direct(screen_mcu_p, screen_mcu_p->i2caddr, buf+16, next_tp_len);
		}
		tp_data = buf + 2;
...
static unsigned screen_mcu_oufei_attach(struct screen_mcu_drv *screen_mcu_p){
    pthread_mutex_lock(&screen_mcu_p->lock);
	info_out("%s, busnum:%d, i2caddr:0x%02x\n", __func__, screen_mcu_p->busnum, screen_mcu_p->i2caddr);
...
static unsigned screen_mcu_oufei_detach(struct screen_mcu_drv *screen_mcu_p){
	info_out("%s, busnum:%d, i2caddr:0x%02x\n", __func__, screen_mcu_p->busnum, screen_mcu_p->i2caddr);
    //pthread_mutex_lock(&screen_mcu_p->lock);
    struct oufei_info *oufei_info_p =  screen_mcu_p->priv_data;
	...
	if(oufei_info_p && oufei_info_p->tp_info){
	    sem_destroy(&oufei_info_p->tp_info->notify_sem);
	    sem_destroy(&oufei_info_p->tp_info->mutex);
	    munmap(oufei_info_p->tp_info, sizeof(screen_mcu_tp_t));
	    oufei_info_p->tp_info = NULL;
	    char shm_name_buf[32]="tp_";
	    strcat(shm_name_buf,screen_mcu_p->name);
		shm_unlink(shm_name_buf);
	}
...
unsigned screen_mcu_oufei_init(struct screen_mcu_drv *screen_mcu_p){
    screen_mcu_p->attach_handler = &screen_mcu_oufei_attach;
    screen_mcu_p->set_request_handler = &set_request_oufei;
	screen_mcu_p->get_reply_handler = &get_reply_oufei;
	screen_mcu_p->detach_handler = &screen_mcu_oufei_detach;
	screen_mcu_p->bl_on = 1;
	screen_mcu_p->bl = 50;
	...
	if(!screen_mcu_p->priv_data){
	    struct oufei_info *oufei_info_p = (struct oufei_info *)malloc(sizeof(struct oufei_info));
	    memset(oufei_info_p, 0x00, sizeof(struct oufei_info));
	    oufei_info_p->tilt_degree = 255;
	    oufei_info_p->tilt_stat = 255;
	    oufei_info_p->power_stat = 255;
	    oufei_info_p->open_stat = 255;
	    screen_mcu_p->priv_data = oufei_info_p;
	}
