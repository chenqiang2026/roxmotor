...
static int set_request_zheyi(void *screen_mcu_drv, char *request){
...
static int get_reply_zheyi(void *screen_mcu_drv, char *reply_msg){
	struct screen_mcu_drv *screen_mcu_p = (struct screen_mcu_drv *)screen_mcu_drv;
	unsigned nbytes=0;
...
static int repo_screen_erro(struct screen_mcu_drv *screen_mcu_p){
	int ret;
    uint8_t buf[2] = {0};
    ret = i2c_read_bytes(screen_mcu_p, screen_mcu_p->i2caddr, 0x05, buf, 2);
    if(!ret && buf[0]==1 && buf[1]!=0){
          info_out("%s, busnum:%d, i2caddr:0x%02x, erro:0x%02x\n", __func__, screen_mcu_p->busnum, screen_mcu_p->i2caddr, buf[1]);
    }
...
static void *request_handler_thread(void *thread_param){
	...
	do{
	    if(screen_mcu_p->status){
			...
			if(screen_mcu_p->busnum == 5){
				repo_screen_erro(screen_mcu_p);
			}
		}
...
static unsigned screen_mcu_zheyi_attach(struct screen_mcu_drv *screen_mcu_p){
...
static unsigned screen_mcu_zheyi_detach(struct screen_mcu_drv *screen_mcu_p){
	info_out("%s, busnum:%d, i2caddr:0x%02x\n", __func__, screen_mcu_p->busnum, screen_mcu_p->i2caddr);

	return 0;
}

unsigned screen_mcu_zheyi_init(struct screen_mcu_drv *screen_mcu_p){
    screen_mcu_p->attach_handler = &screen_mcu_zheyi_attach;
    screen_mcu_p->set_request_handler = &set_request_zheyi;
	screen_mcu_p->get_reply_handler = &get_reply_zheyi;
	screen_mcu_p->detach_handler = &screen_mcu_zheyi_detach;
