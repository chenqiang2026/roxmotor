...
static int set_request_yfmotor(void *screen_mcu_drv, char *request){
...
static int get_reply_yfmotor(void *screen_mcu_drv, char *reply_msg){
	struct screen_mcu_drv *screen_mcu_p = (struct screen_mcu_drv *)screen_mcu_drv;
	unsigned nbytes=0;
	pthread_mutex_lock(&screen_mcu_p->lock);
...
static int handle_request(struct screen_mcu_drv *screen_mcu_p){
    ...
    struct yfmotor_info *yfmotor_info_p = screen_mcu_p->priv_data;
	if(!strcmp(screen_mcu_p->request, "lcd power on")){
	    ...
	    if(!getkeystatus(screen_mcu_p, &tilt_degree, &powerkey_status)){
		 ...
		 snprintf(screen_mcu_p->reply, 32, "%d_%04d_%04d", yfmotor_info_p->sw_ver[0], yfmotor_info_p->sw_ver[1], yfmotor_info_p->sw_ver[2]);
	}else if(!strcmp(screen_mcu_p->request, "lcd get supplier hver")){
		 snprintf(screen_mcu_p->reply, 32, "%d.%d", yfmotor_info_p->hw_ver[0], yfmotor_info_p->hw_ver[1]);
	}else if(!strcmp(screen_mcu_p->request, "lcd get supplier blver")){
		 snprintf(screen_mcu_p->reply, 32, "%d.%d", yfmotor_info_p->bl_ver[0], yfmotor_info_p->bl_ver[1]);
	}else if(!strcmp(screen_mcu_p->request, "lcd get supplier tpver")){
        if(yfmotor_info_p->tp_ver[0] == 0){
            get_version_info(screen_mcu_p);
        }
		snprintf(screen_mcu_p->reply, 32, "%s", yfmotor_info_p->tp_ver);
...
static int get_version_info(struct screen_mcu_drv *screen_mcu_p){
    ...
    struct yfmotor_info *yfmotor_info_p = screen_mcu_p->priv_data;
	do{
		ret = i2c_read_bytes(screen_mcu_p, screen_mcu_p->i2caddr, 0x47, buf, 7);
		if(!ret){
		    //yfmotor_info_p->sw_ver[0] = buf[0];
		    //yfmotor_info_p->sw_ver[1] = buf[1];
		    //yfmotor_info_p->hw_ver[0] = buf[2];
		    //yfmotor_info_p->hw_ver[1] = buf[3];
		    memcpy(yfmotor_info_p->sw_ver, &buf[0], 3);
		    memcpy(yfmotor_info_p->hw_ver, &buf[3], 2);
		    memcpy(yfmotor_info_p->bl_ver, &buf[5], 2);
		    yfmotor_info_p = screen_mcu_p->priv_data;
		    info_out("%s, busnum:%d, i2caddr:0x%02x, %d.%d, %d.%d\n", __func__, screen_mcu_p->busnum, screen_mcu_p->i2caddr, yfmotor_info_p->sw_ver[0], yfmotor_info_p->sw_ver[1], yfmotor_info_p->hw_ver[0], yfmotor_info_p->hw_ver[1]);
		}else{
			break;
		}
		 ret = i2c_read_bytes(screen_mcu_p, screen_mcu_p->i2caddr, 0xa9, buf, 8);
		if(!ret){
		    memcpy(yfmotor_info_p->tp_ver, &buf[0], 7);
		    yfmotor_info_p = screen_mcu_p->priv_data;
		    info_out("%s, busnum:%d, i2caddr:0x%02x, tp_ver:%s\n", __func__, screen_mcu_p->busnum, screen_mcu_p->i2caddr, yfmotor_info_p->tp_ver);
		}else{
			break;
		}
	}
...
#include <time.h>
static int set_time_info(struct screen_mcu_drv *screen_mcu_p){
    static int count = 10;
    int ret = 0;
    count--;
    if(count == 0){
        ...
        if(tm_info->tm_year>=100){
            buf[0] = 0x83;
            buf[1] = tm_info->tm_year-100;
            buf[2] = tm_info->tm_mon+1;
            buf[3] = tm_info->tm_mday;
            buf[4] = tm_info->tm_hour;
            buf[5] = tm_info->tm_min;
            buf[6] = tm_info->tm_sec;
            if(i2c_write_bat(screen_mcu_p, screen_mcu_p->i2caddr, buf, 7)){
                ret = -1;
            }
        }else{
            warn_out("%s %s tm_info->tm_year:%d, tm_info->tm_mon:%d, time erro:%s\n", screen_mcu_p->name, __func__, tm_info->tm_year, tm_info->tm_mon, asctime(tm_info));
            ret = -1;
        }
        count = 60;
    }
    ...
    if(fd >= 0) {
        ...
        if(err == EOK) {
            ...
            if(err == EOK && screen_mcu_p->tp_irq_num > 0) {
				...
				yfmotor_info_p->tp_info = tp_info;
                return 0;
            }
            else {
				err_out("%s, busnum:%d, i2caddr:0x%02x, get gpio %d num %d fail\n", __func__, screen_mcu_p->busnum, screen_mcu_p->i2caddr, screen_mcu_p->tp_irq_gpio, screen_mcu_p->tp_irq_num);
...
static int mcu_ts_read_data(struct screen_mcu_drv *screen_mcu_p){
    int ret = -1;
    struct yfmotor_info *yfmotor_info_p =  screen_mcu_p->priv_data;
    screen_mcu_tp_t *tp_info = yfmotor_info_p->tp_info;
    uint8_t buf[128];
...
static int repo_screen_event(struct screen_mcu_drv *screen_mcu_p, char* key, int value){
    ...
    int fd = open("/var/pps/screen/event_panel2-1", O_RDWR | O_CREAT, S_IRWXU | S_IRWXG | S_IRWXO | O_CLOEXEC);
    if(fd < 0){ 
        warn_out("%s %s can't open /var/pps/screen/event_panel2-1\n", screen_mcu_p->name, __func__);
        return -1;
    }
...
static int get_screen_func_stat(struct screen_mcu_drv *screen_mcu_p){
    uint8_t buf[64];
    int ret = -1;
     ret = i2c_read_bytes(screen_mcu_p, screen_mcu_p->i2caddr, 0x45, buf, 11);
    if(!ret){
...
static int get_screen_voltage_stat(struct screen_mcu_drv *screen_mcu_p){
    uint8_t buf[64];
    int ret = -1;
     ret = i2c_read_bytes(screen_mcu_p, screen_mcu_p->i2caddr, 0x46, buf, 1);
    if(!ret){
...
static int get_screen_dtc_stat(struct screen_mcu_drv *screen_mcu_p){
    uint8_t buf[64];
    int ret = -1;
     ret = i2c_read_bytes(screen_mcu_p, screen_mcu_p->i2caddr, 0x48, buf, 64);
    if(!ret){
...
static int get_screen_event(struct screen_mcu_drv *screen_mcu_p){
    ...
    if(!ret){
        ...
        if(yfmotor_info_p->tilt_degree!=buf[0]){
            yfmotor_info_p->tilt_degree = buf[0];
            repo_screen_event(screen_mcu_p, "ceil_tilt_degree", yfmotor_info_p->tilt_degree);
        }
        if(yfmotor_info_p->tilt_stat!=buf[1]){
            yfmotor_info_p->tilt_stat = buf[1];
            repo_screen_event(screen_mcu_p, "ceil_tilt_stat", yfmotor_info_p->tilt_stat);
        }
        if(yfmotor_info_p->power_stat!=buf[2]){
            yfmotor_info_p->power_stat = buf[2];
            repo_screen_event(screen_mcu_p, "ceil_power_stat", yfmotor_info_p->power_stat);
        }
        if(yfmotor_info_p->open_stat!=buf[3]){
            yfmotor_info_p->open_stat = buf[3];
            repo_screen_event(screen_mcu_p, "ceil_open_stat", yfmotor_info_p->open_stat);
        } 
        if(yfmotor_info_p->anti_pinch_stat!=buf[4]){
            yfmotor_info_p->anti_pinch_stat = buf[4];
            repo_screen_event(screen_mcu_p, "ceil_anti_pinch_stat", yfmotor_info_p->anti_pinch_stat);
        }
         if(yfmotor_info_p->key_lock_stat!=buf[5]){
            yfmotor_info_p->key_lock_stat = buf[5];
            repo_screen_event(screen_mcu_p, "ceil_key_lock_stat", yfmotor_info_p->key_lock_stat);
        }
         if(yfmotor_info_p->key_stat!=buf[6]){
            yfmotor_info_p->key_stat = buf[6];
            repo_screen_event(screen_mcu_p, "ceil_key_stat", yfmotor_info_p->key_stat);
        }
    }
    return ret;
}
	...
	while(1){
        ...
        if(!i2c_read_bytes(screen_mcu_p, screen_mcu_p->i2caddr, 0x4f, &buf, 1)){
            ...
            if(buf == 0x41){
                ...
                get_screen_voltage_stat(screen_mcu_p);
            }else if(buf == 0x48){
                get_screen_dtc_stat(screen_mcu_p);
            }
         }
         pthread_mutex_unlock(&screen_mcu_p->lock);
	}
...
static unsigned screen_mcu_yfmotor_attach(struct screen_mcu_drv *screen_mcu_p){
	...
	if(!tp_irq_init(screen_mcu_p)){
		...
		if(0!= pthread_getschedparam(screen_mcu_p->tp_irq_thread,&policy,&param))
        {
            ...
            pthread_setname_np(screen_mcu_p->tp_irq_thread,"yfmotor_irq_thread_worker");
...
static unsigned screen_mcu_yfmotor_detach(struct screen_mcu_drv *screen_mcu_p){
	info_out("%s, busnum:%d, i2caddr:0x%02x\n", __func__, screen_mcu_p->busnum, screen_mcu_p->i2caddr);
    //pthread_mutex_lock(&screen_mcu_p->lock);
    struct yfmotor_info *yfmotor_info_p =  screen_mcu_p->priv_data;
	...
	if(yfmotor_info_p && yfmotor_info_p->tp_info){
	    sem_destroy(&yfmotor_info_p->tp_info->notify_sem);
	    sem_destroy(&yfmotor_info_p->tp_info->mutex);
	    munmap(yfmotor_info_p->tp_info, sizeof(screen_mcu_tp_t));
	    yfmotor_info_p->tp_info = NULL;
	    char shm_name_buf[32]="tp_";
	    strcat(shm_name_buf,screen_mcu_p->name);
		shm_unlink(shm_name_buf);
	}
...
unsigned screen_mcu_yfmotor_init(struct screen_mcu_drv *screen_mcu_p){
    screen_mcu_p->attach_handler = &screen_mcu_yfmotor_attach;
    screen_mcu_p->set_request_handler = &set_request_yfmotor;
	screen_mcu_p->get_reply_handler = &get_reply_yfmotor;
	screen_mcu_p->detach_handler = &screen_mcu_yfmotor_detach;
	screen_mcu_p->bl_on = 1;
	screen_mcu_p->bl = 50;
	...
	if(!screen_mcu_p->priv_data){
	    struct yfmotor_info *yfmotor_info_p = (struct yfmotor_info *)malloc(sizeof(struct yfmotor_info));
	    memset(yfmotor_info_p, 0x00, sizeof(struct yfmotor_info));
	    yfmotor_info_p->tilt_degree = 255;
	    yfmotor_info_p->tilt_stat = 255;
	    yfmotor_info_p->power_stat = 255;
	    yfmotor_info_p->open_stat = 255;
	    yfmotor_info_p->anti_pinch_stat = 255;
	    yfmotor_info_p->key_lock_stat = 255;
	    yfmotor_info_p->key_stat = 255;
	    screen_mcu_p->priv_data = yfmotor_info_p;
	}
